...
macro_rules! elf_define_type {
    ($(#[$meta:meta])* pub $name:ident, $inner:ty) => {
        ...
        impl $name {
            ...
            pub async fn read(offset:&mut u64, file : &$crate::fs::DynFile, endianness:Endianness) -> $crate::Result<$name> {
                use $crate::fs::File;
            ...
            pub async fn read_buffer(offset:&mut u64, file : &$crate::fs::DynFile, endianness:Endianness, buffer : &mut $name) -> $crate::Result<u64> {
                use $crate::fs::File;
                use $crate::data_buffer;
                let mut bytes = [0u8; <$name>::SIZE_BYTES];
                let length = file.read(offset, &mut data_buffer::Kernel::new(&mut bytes)).await?;
            ...
            pub async fn read_vector(offset:&mut u64, file : &$crate::fs::DynFile, endianness:Endianness, vector: &mut alloc::vec::Vec<$name>) -> $crate::Result<usize> {
                let mut byte_counter = 0;
                for _ in  0..vector.len(){
                    let mut value = $name::default();
                    match $name::read_buffer(offset, file, endianness, &mut value).await {
                        Ok(length) => {
                            byte_counter += length;
