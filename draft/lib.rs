...
impl ELF<'_> {
    ...
    pub unsafe fn load_filepath(filepath: *const u8) -> (usize, Option<usize>) {
        ...
        let program_origin = elf.load_program().unwrap();

        let interpreter_filepath = elf.load_interpreter().unwrap();
        let interpreter_origin = if interpreter_filepath != usize::MAX {
            let elf = ELF::read_filepath(interpreter_filepath as *mut u8).unwrap();
            Some(elf.load_program().unwrap())
        } else {
            None
        };
    ...
    pub unsafe fn load(&self) -> (usize, Option<usize>) {
        let program_origin = self.load_program().unwrap();

        let interpreter_filepath = self.load_interpreter().unwrap();
        let interpreter_origin = if interpreter_filepath != usize::MAX {
            let elf = ELF::read_filepath(interpreter_filepath as *mut u8).unwrap();
            Some(elf.load_program().unwrap())
        } else {
    ...
    pub unsafe fn load_interpreter(&self) -> Result<usize> {
        if let Some(program_table) = &self.program_table {
            x86_64::print_str("\nProgram Table\n");
            let interpreter_headers = program_table.headers.iter().filter(|hdr| hdr.ptype.0 == 3);
