...
impl ELF {
    ...
    pub fn load_programs(&self) -> *mut libc::c_void {
        ...
        for program_segment in program_segments {
            ...
            let segment_upper_file_address =
                (segment_header.offset + segment_header.filesz) as usize;
            let segment_file_address_slice =
                segment_lowest_file_address..segment_upper_file_address;
    ...
    pub fn load_interpreters(&self) -> () {
        let mut interpreter_segments: Vec<program::Entry> = self
            .program_header_table
            .clone()
            .unwrap()
            .entries
    ...
    pub fn execute<P: AsRef<std::path::Path>>(filepath: P) -> ! {
        ...
        println!("Entry point at: 0x{:x}", elf_program_entry_address);

        let entry: unsafe extern "C" fn() -> ! =
            unsafe { core::mem::transmute(elf_program_entry_address) };
        unsafe { entry() }
    }
